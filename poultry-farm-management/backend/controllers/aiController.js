...
const chatWithAssistant = async (req, res) => {
  try {
    ...
    **Smart Suggestions Rule:**
    ...
    // History must strictly alternate 'user' and 'model', starting with 'user'
    let formattedHistory = [];
    let currentRole = 'user';
    let currentParts = [];

    // Combine any previous history and the new message into a flat array of messages
    ...
    for (const msg of allMessages) {
      ...
      if (msgRole === currentRole) {
        ...
        if (currentParts.length > 0) {
          formattedHistory.push({ role: currentRole, parts: currentParts });
        }
        currentRole = msgRole;
