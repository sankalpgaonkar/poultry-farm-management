  ...
  try {
    ...
    **Smart Suggestions Rule:**
    ...
    for (const msg of allMessages) {
      ...
      if (collapsedMessages.length > 0 && collapsedMessages[collapsedMessages.length - 1].role === currentRole) {
        collapsedMessages[collapsedMessages.length - 1].content += "\n\n" + msg.content;
      } else {
        collapsedMessages.push({ role: currentRole, content: msg.content });
      }
    }

    // Ensure starts with user
    if (collapsedMessages.length > 0 && collapsedMessages[0].role !== 'user') {
      collapsedMessages.shift();
    }

    // Map to Gemini format
    const contents = collapsedMessages.map(msg => ({
      role: msg.role,
