...
const chatWithAssistant = async (req, res) => {
  try {
    ...
    **Smart Suggestions Rule:**
    ...
    // Create contents list from history
    let rawContents = [];
    history.forEach(h => {
       rawContents.push({
    ...
    // Fix consecutive messages and ensure starts with user
    let contents = [];
    ...
    for (let i = 0; i < rawContents.length; i++) {
       if (currentRole === null) {
          if (rawContents[i].role === 'model') {
             // model can't start first, insert dummy user
             contents.push({ role: 'user', parts: [{ text: "Hello" }] });
          }
          currentRole = rawContents[i].role;
          currentParts = [...rawContents[i].parts];
       } else if (rawContents[i].role === currentRole) {
