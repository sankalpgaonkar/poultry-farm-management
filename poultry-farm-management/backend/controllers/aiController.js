  ...
  try {
    ...
    **Smart Suggestions Rule:**
    ...
    for (const h of history) {
      rawParts.push({ role: h.role === 'user' ? 'user' : 'model', content: h.content });
    }
    rawParts.push({ role: 'user', content: message });

    // Collapse consecutive same roles
    const collapsedParts = [];
    for (const part of rawParts) {
      if (collapsedParts.length > 0 && collapsedParts[collapsedParts.length - 1].role === part.role) {
        collapsedParts[collapsedParts.length - 1].content += "\n" + part.content;
      } else {
    ...
    if (collapsedParts.length > 0 && collapsedParts[0].role !== 'user') {
      collapsedParts.unshift({ role: 'user', content: "Hello" });
    }

    const contents = collapsedParts.map(part => ({
      role: part.role,
