...
const chatWithAssistant = async (req, res) => {
  try {
    ...
    **Smart Suggestions Rule:**
    ...
    // Ensure strict alternation and start with 'user' role
    ...
    const addMessage = (role, text) => {
      if (lastRole === role) {
        ...
        if (lastRole !== null) {
          processedHistory.push({ role: lastRole, parts: [{ text: accumulatedText }] });
        }
        lastRole = role;
    ...
    history.forEach(h => {
      ...
      addMessage(role, h.content);
    });

    // Add the new message
    addMessage('user', message);

    // Push the final accumulated message
    if (lastRole !== null) {
      processedHistory.push({ role: lastRole, parts: [{ text: accumulatedText }] });
    }
    ...
    if (processedHistory.length > 0 && processedHistory[0].role !== 'user') {
       processedHistory.unshift({ role: 'user', parts: [{ text: "(Context)" }] });
    }
