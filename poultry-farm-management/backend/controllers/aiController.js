...
const chatWithAssistant = async (req, res) => {
  try {
    ...
    **Smart Suggestions Rule:**
    ...
    // Ensure alternating roles starting with 'user', collapsing consecutive roles
    let collapsedContents = [];
    let currentRole = null;
    let currentParts = [];

    for (let i = 0; i < rawContents.length; i++) {
      if (rawContents[i].role === currentRole) {
        currentParts.push(...rawContents[i].parts);
      } else {
        if (currentRole !== null) {
          collapsedContents.push({ role: currentRole, parts: currentParts });
        }
        currentRole = rawContents[i].role;
        currentParts = [...rawContents[i].parts];
      }
    }
    if (currentRole !== null) {
      collapsedContents.push({ role: currentRole, parts: currentParts });
    ...
    if (collapsedContents.length > 0 && collapsedContents[0].role !== 'user') {
      collapsedContents.unshift({ role: 'user', parts: [{ text: 'Hello' }] });
    }
