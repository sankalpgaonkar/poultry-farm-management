...
const chatWithAssistant = async (req, res) => {
  try {
    ...
    **Smart Suggestions Rule:**
    ...
    // Ensure strictly alternating sequence starting with 'user'
    let contents = [];
    for (const item of rawContents) {
      if (!item.parts[0].text) continue; // Skip empty messages
      if (contents.length === 0) {
        if (item.role === 'model') {
           // If first item is model, we prepend a dummy user message or drop it. Dropping it is safer.
           continue;
        }
        contents.push(item);
      } else {
