...
const chatWithAssistant = async (req, res) => {
  try {
    ...
    **Smart Suggestions Rule:**
    ...
    // Collapse consecutive roles and ensure strictly alternating user and model roles
    let contents = [];
    for (let i = 0; i < rawContents.length; i++) {
      if (contents.length > 0 && contents[contents.length - 1].role === rawContents[i].role) {
        // Append text to the previous part
