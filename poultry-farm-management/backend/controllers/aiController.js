...
const chatWithAssistant = async (req, res) => {
  try {
    ...
    **Smart Suggestions Rule:**
    ...
    const allMessages = [...rawHistory, { role: 'user', parts: [{ text: message }] }];

    // Collapse consecutive identical roles
    const contents = [];
    for (const msg of allMessages) {
      if (contents.length > 0 && contents[contents.length - 1].role === msg.role) {
        // If the roles are identical, concatenate the text with a newline
        const lastPart = contents[contents.length - 1].parts[0];
        lastPart.text = lastPart.text + "\n" + msg.parts[0].text;
      } else {
        // Otherwise, add a new message to contents
        // Ensure deep copy to avoid mutations
