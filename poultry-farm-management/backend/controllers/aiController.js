...
const chatWithAssistant = async (req, res) => {
  try {
    ...
    **Smart Suggestions Rule:**
    ...
    // Ensure history roles strictly alternate and start with 'user'
    const allMessages = history.map(h => ({
      role: h.role === 'user' ? 'user' : 'model',
      text: h.content
    }));
    allMessages.push({ role: 'user', text: message });
    ...
    for (const msg of allMessages) {
      if (msg.role === currentRole) {
        ...
        if (currentRole !== null) {
          collapsedContents.push({ role: currentRole, parts: [{ text: currentText }] });
        }
