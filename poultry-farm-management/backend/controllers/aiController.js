...
const initGenAI = async () => {
  if (!aiClient && process.env.GEMINI_API_KEY && process.env.GEMINI_API_KEY !== 'dummy_key_for_now') {
    try {
      ...
      aiModel = "gemini-2.0-flash"; // Stable free tier. Use "gemini-3-flash-preview" if enabled.
    } catch (err) {
...
const chatWithAssistant = async (req, res) => {
  try {
    ...
    **Smart Suggestions Rule:**
    ...
    // Ensure alternating history strictly starts with a 'user' message
    if (processedHistory.length > 0 && processedHistory[0].role === 'model') {
      processedHistory.unshift({ role: 'user', parts: [{ text: 'Hi' }] });
    }

    // Collapse consecutive roles into a single message
    ...
    // Prepare final contents with alternating roles and the latest message
    const contents = [...collapsedHistory];
