...
const chatWithAssistant = async (req, res) => {
  try {
    ...
    **Smart Suggestions Rule:**
    ...
    // Combine history to ensure strictly alternating roles starting with 'user'
    let mappedHistory = [];
    let lastRole = null;
    ...
    for (const h of allMessages) {
        if (!h.text) continue; // Skip empty messages
        if (h.role === lastRole) {
            mappedHistory[mappedHistory.length - 1].parts[0].text += "\n" + h.text;
        } else {
            mappedHistory.push({ role: h.role, parts: [{ text: h.text }] });
            lastRole = h.role;
        }
    }
    ...
    if (mappedHistory.length > 0 && mappedHistory[0].role !== 'user') {
        mappedHistory.unshift({ role: 'user', parts: [{ text: "Hello" }] });
    }
