...
const chatWithAssistant = async (req, res) => {
  try {
    ...
    **Smart Suggestions Rule:**
    ...
    // History needs to start with a 'user' and alternate roles
    const combinedInput = [...history, { role: 'user', content: message }];

    let collapsedHistory = [];
    for (const h of combinedInput) {
        const mappedRole = h.role === 'user' ? 'user' : 'model';
        if (collapsedHistory.length > 0 && collapsedHistory[collapsedHistory.length - 1].role === mappedRole) {
