...
const chatWithAssistant = async (req, res) => {
  try {
    ...
    **Smart Suggestions Rule:**
    ...
    // Alternate roles strictly starting with 'user'
    ...
    for (const msg of mappedHistory) {
      if (msg.role !== expectedRole) {
        if (expectedRole === 'user') {
          // Missing user message, insert dummy
          formattedContents.push({ role: 'user', parts: [{ text: '...' }] });
          // Then push the actual model message
          formattedContents.push(msg);
        } else {
          // Two user messages in a row? Collapse them
